    <header>
    <?php 
        require_once("inc/head.php");
        require_once("inc/header.php"); 
    ?>
    </header>
    <main>
        <div class="container-fluid">
            <div class="row">
                <?php require_once("inc/menu.php"); ?>
            </div>

            <div id="galeria" class="col-10 col-md-10">
                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 1</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>

                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 2</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>

                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 3</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>


                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 4</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>


                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 5</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>

                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 6</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>

                <div class="video">
                    <div class="thumbnail">
                        <img src="https://img.youtube.com/vi/M7lc1UVf-VE/hqdefault.jpg" alt="">
                        <div class="overlay">
                            <a href="player.php" class="icon" title="User Profile">
                                <i class="fa fa-play-circle"></i>
                            </a>
                        </div>
                    </div>

                    <strong>Vídeo 7</strong>
                    <p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Adipisci quo corrupti,
                        necessitatibus architecto sed autem consequatur hic blanditiis.</p>
                </div>
            </div>
        </div>
        </div>
    </main>

    <footer class="footer">
        <?php require_once("inc/footer.php"); ?>
    </footer>

</body>

</html>